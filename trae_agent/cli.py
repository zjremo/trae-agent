...
def create_agent(config: Config) -> TraeAgent:
    ...
    try:
        # Create agent
        agent = TraeAgent(LLMProvider(config.default_provider), config.model_providers[config.default_provider], config.max_steps)
        return agent
...
def run(task: str, provider: str | None = None, model: str | None = None, api_key: str | None = None, 
        context: str | None = None, max_steps: int | None = None, verbose: bool = False, 
        working_dir: str | None = None, must_patch: bool = False, config_file: str = "trae_config.json",
    ...
    [bold]Working Directory:[/bold] {os.getcwd()}
     ...
     [bold]Trajectory File:[/bold] {trajectory_path}""",
        title="Trae Agent Execution",
        border_style="blue"
    ))
    
    # Configure agent for progress display
    agent.set_progress_display(show_progress=True)
    ...
    try:
        ...
        execution = asyncio.run(agent.execute_task())
