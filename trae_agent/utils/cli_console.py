...
class CLIConsole:
    ...
    def __init__(self, config: Config | None):
        ...
        self.lakeview_config: LakeviewConfig | None = config.lakeview_config if config is not None and config.enable_lakeview else None
        self.lake_view: LakeView | None = LakeView(config) if config is not None and config.enable_lakeview else None
    ...
    async def start(self):
        while True:
            if self.agent_execution and \
                (self.lake_view is None or \
                (len(self.agent_execution.steps) == len(self.console_steps) and \
                    all(step.lake_view_generator_done for step in self.console_steps.values()))):
                break
    ...
    def print_task_details(self, task: str, working_dir: str, provider: str, model: str, max_steps: int, config_file: str, trajectory_file: str):
...
[bold]Trajectory File:[/bold] {trajectory_file}""",
    ...
    def _create_compact_step_display(self, agent_step: AgentStep):
        ...
        step_content.append(f"[{color}]{emoji} Step {agent_step.step_number}: {agent_step.state.value.title()}[/{color}]")
        ...
        if agent_step.tool_calls:
            ...
            if agent_step.tool_results:
                ...
                status = "[green]✅" if success_count == total_count else f"[yellow]⚠️ {success_count}/{total_count}[/yellow]"
...
{lake_view_step.desc_details}""",
    ...
    def create_agent_steps_display(self):
        ...
        if self.agent_execution is None:
            previous_steps = self.agent_step_history[:-1] if len(self.agent_step_history) >=2 else []
            ...
            for step in previous_steps:
                ...
                if step_id not in self.console_steps:
                    ...
                    if self.lake_view is not None:
                        lake_view_panel_generator = asyncio.create_task(self._create_lakeview_step_display(step))
                    else:
                    ...
                    if self.lake_view is None:
                        ...
                        if console_step.lake_view_panel_generator is not None:
                            if console_step.lake_view_panel_generator.done():
                                lake_view_panel = console_step.lake_view_panel_generator.result() or console_step.panel
                                panels.append(lake_view_panel)
                                self.console_steps[step_id] = ConsoleStep(lake_view_panel, None, True)
                            else:
                                panels.append(console_step.panel)
                        else:
                            panels.append(console_step.panel)
